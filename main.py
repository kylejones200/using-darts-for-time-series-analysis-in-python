...
def main():
    ...
    if data_path:
        ...
        raise ValueError(
            "Must provide either --data-path or --api-key (or set in config)"
        )
...
plot_forecast(
    series, forecast, "ARIMA Forecast", output_dir / "arima_forecast.png", metrics
)
...
metrics = evaluate_forecast(val, forecast)
logging.info(
    f"Exponential Smoothing - MAE: {metrics['mae']:.2f}, MAPE: {metrics['mape']:.2%}"
)
plot_forecast(
    series,
    forecast,
    "Exponential Smoothing Forecast",
    output_dir / "exponential_smoothing_forecast.png",
    metrics,
)
...
plot_forecast(
    series, forecast, "LightGBM Forecast", output_dir / "lightgbm_forecast.png", metrics
)
...
plot_forecast(
    series, forecast, "LSTM Forecast", output_dir / "lstm_forecast.png", metrics
)
...
plot_forecast(
    series, forecast, "NBEATS Forecast", output_dir / "nbeats_forecast.png", metrics
)
...
plot_forecast(
    series, forecast, "FFT Forecast", output_dir / "fft_forecast.png", metrics
)
