...
def fetch_fred_series(series_id: str, start: str='2000-01-01', end=None) -> TimeSeries:
...
def load_data(data_path: Path | None=None, series_id: str | None=None, start: str='2000-01-01') -> TimeSeries:
    """Load time series data from file or FRED via pandas_datareader."""
    if data_path and data_path.exists():
        df = pd.read_csv(data_path, parse_dates=['date'], index_col='date')
        return TimeSeries.from_dataframe(df, value_cols=['value'])
    elif series_id:
        return fetch_fred_series(series_id, start=start)
    raise ValueError('Either data_path or series_id must be provided')
...
def fit_lstm(train: TimeSeries, val: TimeSeries=None, input_chunk_length: int=30, output_chunk_length: int=7, n_epochs: int=50) -> RNNModel:
    """Fit LSTM model."""
    model = RNNModel(model='LSTM', input_chunk_length=input_chunk_length, output_chunk_length=output_chunk_length, n_epochs=n_epochs)
    model.fit(train, val_series=val)
    return model

def fit_nbeats(train: TimeSeries, val: TimeSeries=None, input_chunk_length: int=30, output_chunk_length: int=7, n_epochs: int=50, random_state: int=42) -> NBEATSModel:
    """Fit NBEATS model."""
    torch_kwargs = {'pl_trainer_kwargs': {'accelerator': 'cpu', 'callbacks': [TFMProgressBar(enable_train_bar_only=True)]}}
    model = NBEATSModel(input_chunk_length=input_chunk_length, output_chunk_length=output_chunk_length, n_epochs=n_epochs, random_state=random_state, **torch_kwargs)
    model.fit(train, val_series=val)
    return model
...
def evaluate_forecast(actual: TimeSeries, forecast: TimeSeries) -> Dict[str, float]:
    ...
    return {'mae': mae(actual, forecast), 'mape': mape(actual, forecast), 'r2': r2_score(actual, forecast)}

def plot_forecast(series: TimeSeries, forecast: TimeSeries, title: str, output_path: Path, metrics: Dict[str, float] | None=None, plot: bool=True):
