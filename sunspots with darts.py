...
from darts.models import TBATS, ExponentialSmoothing, LinearRegressionModel, NHiTSModel, RandomForest, RNNModel, Theta
from darts.models import TBATS, ExponentialSmoothing, LinearRegressionModel, NHiTSModel, RandomForest, Theta
...
def data_preparation() -> None:
    ...
    year_series = datetime_attribute_timeseries(pd.date_range(start=series.start_time(), freq=series.freq_str, end=series.end_time()), attribute='year', one_hot=False)

    covariates = Scaler().fit_transform(year_series)

    models = {'Exponential Smoothing': ExponentialSmoothing(trend=ModelMode.ADDITIVE, seasonal=SeasonalityMode.ADDITIVE, seasonal_periods=124), 'Theta': Theta(seasonality_period=124, season_mode=SeasonalityMode.ADDITIVE), 'Linear Regression': LinearRegressionModel(lags=124, output_chunk_length=20), 'Random Forest': RandomForest(lags=[-1, -12, -124], output_chunk_length=20, n_estimators=200, max_depth=20, min_samples_split=10, criterion='absolute_error'), 'RNN': RNNModel(model='LSTM', training_length=240, input_chunk_length=120, n_epochs=200, batch_size=20, optimizer_kwargs={'lr': 0.001}), 'TBATS': TBATS(seasonal_periods=[12], use_box_cox=True, use_trend=True, use_arma_errors=True), 'NHiTS': NHiTSModel(input_chunk_length=100, output_chunk_length=20, n_epochs=80)}
    ...
    for name, color in zip(models.keys(), colors):
        line, = ax.plot([], [], label=f"{name} (MAPE: {results[name]['mape']}%)", lw=2, color=color)
        lines[name] = line
    ...
    anim = animation.FuncAnimation(fig, animate, frames=len(models), interval=1000, blit=True, repeat=True)
...
def previous_code_remains_the_same_until_the_animati() -> None:
    fig = plt.figure(figsize=(15, 7))
    ...
    historical_line, = ax.plot(all_times[historical_mask], all_values[historical_mask], label='Historical', color='gray', alpha=0.6)
    ...
    anim = animation.FuncAnimation(fig, animate, frames=len(val), interval=50, blit=True, repeat=True)
...
def load_and_prepare_the_data() -> None:
...
def data_preparation_2() -> None:
    ...
    models = {'Exponential Smoothing': ExponentialSmoothing(trend=ModelMode.ADDITIVE, seasonal=SeasonalityMode.ADDITIVE, seasonal_periods=11), 'Theta': Theta(seasonality_period=11, season_mode=SeasonalityMode.ADDITIVE), 'Linear Regression': LinearRegressionModel(lags=11, output_chunk_length=20), 'Random Forest': RandomForest(lags=[-1, -2, -11], output_chunk_length=20, n_estimators=200, max_depth=20, min_samples_split=10, criterion='absolute_error'), 'TBATS': TBATS(seasonal_periods=[11], use_box_cox=True, use_trend=True, use_arma_errors=True), 'NHiTS': NHiTSModel(input_chunk_length=10, output_chunk_length=20, n_epochs=80)}
    ...
    ax.plot(historical_df.index, historical_df.values, label='Historical', color='black', alpha=0.6)
    ...
    anim = animation.FuncAnimation(fig, animate, frames=20, interval=200, blit=True, repeat=True)
...
def data_preparation_3() -> None:
    ...
    plt.plot(historical_df.index, historical_df.values, label='Historical', color='black', alpha=0.6, linewidth=2)
    ...
    plt.text(prediction_start, plt.ylim()[1], 'Prediction Start', rotation=90, verticalalignment='top')
...
def calculate_number_of_years_to_predict() -> None:
...
def calculate_number_of_years_to_predict_2() -> None:
    # Calculate number of years to predict (from 1980 to 2026)
    end_date = pd.Timestamp("20260101")
    train_end = train.time_index[-1]
    years_to_predict = (end_date.year - train_end.year) + 1
...
def main() -> None:
    data_preparation()
    previous_code_remains_the_same_until_the_animati()
    load_the_datasets()
    load_and_prepare_the_data()
    data_preparation_2()
    data_preparation_3()
    calculate_number_of_years_to_predict()
    calculate_number_of_years_to_predict_2()
